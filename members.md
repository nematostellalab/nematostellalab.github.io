---
title: "Lab Members"
permalink: /members/
layout: single
author_profile: false
---

## Lab Members

**Whitney Leach, PhD**
<h2>Anemone Supplier</h2>

<div style="max-width:600px; display:flex; gap:20px; align-items:center;">

  <img src="/images/whitney.jpg" style="width:180px; border-radius:12px;">

  <div>
    <h3 style="margin-bottom:5px;">Whitney Leach, PhD</h3>
    <p style="margin:5px 0;">Assistant Professor, DeSales University</p>
    <p style="font-size:14px; color:gray;">
      Studying circadian systems, behavior, and gene regulation in <i>Nematostella vectensis</i>.
    </p>
  </div>

</div>
---

### Research Students
<h3>Research Students</h3>

<div style="display: flex; flex-wrap: wrap; gap: 25px;">

  <div style="width:220px;">
    <h4>Mikayla Lighting</h4>
    <p style="font-size:14px;">Investigating mechanisms of sex determination in <i>Nematostella vectensis</i>.</p>
  </div>

  <div style="width:220px;">
    <h4>Chahinez Hennous</h4>
    <p style="font-size:14px;">Studying mucus production in <i>Nematostella vectensis</i>.</p>
  </div>

  <div style="width:220px;">
    <h4>Ayah Suleiman</h4>
    <p style="font-size:14px;">Research on mucus production in <i>Nematostella vectensis</i>.</p>
  </div>

  <div style="width:220px;">
    <h4>Jess Nahon</h4>
    <p style="font-size:14px;">Researching ways to induce mucus production in <i>Nematostella vectensis</i>.</p>
  </div>

  <div style="width:220px;">
    <h4>Natalie Skuza</h4>
    <p style="font-size:14px;">Studying circadian differences in <i>Nematostella vectensis</i> using Raspberry Pi systems.</p>
  </div>

  <div style="width:220px;">
    <h4>Hadeer Hamada</h4>
    <p style="font-size:14px;">Research on mucus production in <i>Nematostella vectensis</i>.</p>
  </div>

  <div style="width:220px;">
    <h4>Natalie Martin</h4>
    <p style="font-size:14px;">Investigating sex differences in behavioral rhythms in cnidarians.</p>
  </div>

  <div style="width:220px;">
    <h4>Vanessa Ferriolo</h4>
    <p style="font-size:14px;">Research on circadian and behavioral biology.</p>
  </div>

</div>

---

## About the Team

Our lab is an undergraduate- and trainee-focused research group working on behavior, circadian biology, and marine model systems using *Nematostella vectensis* and Raspberry Pi systems.
